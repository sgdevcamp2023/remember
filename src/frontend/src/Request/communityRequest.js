...
export const getGuildListRequest = async (userId) => {
  return await axios.get(CommunityServerAddr + `api/community/check/guild/${userId}`, {
    headers: {
...
export const getChannelListRequest = async (guildId, userId) => {
  return await axios
    .get(CommunityServerAddr + `api/community/check/guild/channel/${guildId}/${userId}`,
      {
        headers: {Authorization: `Bearer ${accessToken}`},
      }
    )
...
export const getUserStateAndVoice = async (CURRENT_VIEW_GUILD, USER_ID) => {
  return await axios.get(CommunityServerAddr + `api/community/guild/${CURRENT_VIEW_GUILD}/${USER_ID}`, {
    headers: {
      Authorization: `Bearer ${accessToken}`,
    },
  })
