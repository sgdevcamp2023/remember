...
export default function Guilds() {
  ...
  const appendServer = "https://storage.googleapis.com/remember-harmony/fe2f6651-10c4-444c-a336-3740dd4a5890";
  const userId = 1;
  ...
  const handleServerAdd = (event) => {
    ...
    axios.post(apiAddr + "/api/community/registration/guild", formData, {
      headers: {
  ...
  return (
    <div className="sidebar">
      <Link to={`/main`} onClick={() => {
        ...
        axios.get(apiAddr + `/api/community/check/room/${userId}`, {
          headers: {
            "Authorization": bearerToken
          }
        })
      ...
      {Guilds.length > 0 ? Guilds.map((guild, index) => (
        <Link to={`/guild/${guild.guildId}`} onClick={() => {
          removeChannels();
          removeCategories();
          axios.get(apiAddr + `/api/community/check/category/${guild.guildId}/${userId}`, {
            headers: {
              "Authorization": bearerToken
            }
          })
          ...
          axios.get(apiAddr + `/api/community/check/guild/channel/${guild.guildId}/${userId}`, {
            headers: {
              "Authorization": bearerToken
            }
          })
      ...
      >
        <div className="server-add-modal-header">
          <h2>서버 커스터마이징하기</h2>
          <h4>새로운 서버에 이름과 아이콘을 부여해 개성을 드러내 보세요. 나<br/>중에 언제든 바꿀 수 있어요.</h4>
        </div>
