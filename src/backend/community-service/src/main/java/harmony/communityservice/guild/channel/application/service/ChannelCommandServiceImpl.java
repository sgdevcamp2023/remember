...
public class ChannelCommandServiceImpl implements ChannelCommandService {
    ...
    public Long register(RegisterChannelRequest registerChannelRequest) {
        ChannelEntity channel = ToChannelMapper.convert(registerChannelRequest);
    ...
    public void delete(DeleteChannelRequest deleteChannelRequest) {
        channelCommandRepository.deleteById(ChannelIdJpaVO.make(deleteChannelRequest.channelId()));
    ...
    public void deleteByGuildId(Long guildId) {
        List<ChannelIdJpaVO> channelIds = channelCommandRepository.findIdsByGuildIdAndType(GuildIdJpaVO.make(guildId),
                ChannelTypeJpaEnum.FORUM);
        Events.send(new DeleteBoardsInGuildEvent(channelIds));
        channelCommandRepository.deleteByGuildId(GuildIdJpaVO.make(guildId));
        for (ChannelIdJpaVO channelId : channelIds) {
